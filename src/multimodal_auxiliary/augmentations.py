...
class SentinelAugmentations:
    def __init__(
        ...
        self.spatial_transform = A.Compose(
            ...
            additional_targets={
                ...
                "ignition_pt": "mask",
                "era5_image": "image",
                # Landcover is categorical -> treat as mask so transforms use
                # nearest-neighbor interpolation (avoid fractional classes).
                "landcover_np": "mask",
                "mask": "mask",
            },
        )
    ...
    @staticmethod
    def _to_landcover_tensor(lc: np.ndarray) -> torch.Tensor:
        """Convert landcover mask to [1,H,W] long tensor (class indices)."""
        if lc.ndim == 3:
            lc = lc[..., 0]
        # Do NOT binarize; landcover is multi-class.
        return torch.from_numpy(lc.astype(np.int64)).unsqueeze(0)
    ...
    def __call__(
        ...
        # landcover_final is categorical
        landcover_tensor = self._to_landcover_tensor(landcover_final)
