...
def process_single_fire(fire_id, config):
    """
    Processa un singolo incendio, cercando immagini Landsat *prima* dell'incendio,
    ...
    stac_collections = ["landsat-c2-l2"]
    ...
    for i, item_candidate in enumerate(items):
        ...
        if ref_band_name not in item_candidate.assets:
             ref_band_name = next((b for b, r in landsat_band_resolutions.items() if r == 30 and b in item_candidate.assets), None)
    ...
    if not valid_items:
        print("🔴 Nessuna delle immagini Landsat trovate interseca la geometria dell'incendio o ha bande valide. Impossibile procedere con il ritaglio.")
        print(f"\n--- FINE Processo per incendio ID: {fire_id} ---")
        return
        ...
        with rasterio.open(signed_href) as ref_src:
            ...
            if current_date_str in processed_images_by_date:
                ...
                for existing_30m_bands_stack in processed_images_by_date[current_date_str]:
                    ...
                    if current_30m_bands_stack is not None and are_images_similar(current_30m_bands_stack, existing_30m_bands_stack, tolerance_percentage=image_similarity_tolerance): 
            ...
            # Il nome del file Landsat includerà la data dell'immagine e un indice sequenziale per l'incendio
            image_filename = f"fire_{fire_id}_{current_date_str}_pre_landsat_{image_idx_in_fire}.tif"
            ...
            if not gt_generated_for_fire:
                if gt_geometry_utm: 
                    ...
                    try:
                        ...
                        out_path_gt_tif = os.path.join(FIRE_SAVE_FOLDER, f"fire_{fire_id}_GTLandsat.tif") 
                        ...
                        out_path_gt_png = os.path.join(FIRE_SAVE_FOLDER, f"fire_{fire_id}_GTLandsat.png")
...
if __name__ == "__main__":
    ...
    # Filtra gli incendi per data 
    gdf_all_fires_filtered = gdf_all_fires[pd.to_datetime(gdf_all_fires["initialdate"]) >= min_date_for_Landsat2]
    
    print(f"Trovati {len(gdf_all_fires)} incendi nel GeoJSON. Filtrati {len(gdf_all_fires_filtered)} incendi dopo il {min_date_for_Landsat2.strftime('%Y-%m-%d')}.")
